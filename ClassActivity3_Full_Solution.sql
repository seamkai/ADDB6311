...
SELECT (c.First_Name||', '||c.Surname) AS CUSTOMER, s.Staff_ID AS STAFF_ID, dd.Driver_ID AS DRIVER_ID, di.Description AS DESCRIPTION
FROM Customer c INNER JOIN Billing b  ON c.Customer_Id = b.Customer_Id
INNER JOIN Staff s ON s.Staff_ID = b.Staff_ID
INNER JOIN Delivery_Items di ON s.Staff_ID = di.Staff_ID
...
FROM Driver d INNER JOIN Driver_Deliveries dd ON d.Driver_ID = dd.Driver_ID
INNER JOIN Delivery_Items di ON di.delivery_item_id = dd.delivery_item_id
INNER JOIN Vehicle v ON v.vin_number = dd.vin_number
WHERE d.Driver_ID = 81011;


--Question 5 No Cursor Declaration
...
BEGIN
    SELECT Vehicle_Type, Mileage, Manufacturer INTO    v_Vehicle, v_Mileage, v_Manufacturer
    FROM vehicle
    WHERE mileage > 700000;


    DBMS_OUTPUT.PUT_LINE('Vehicle:         ' ||v_Vehicle);
    DBMS_OUTPUT.PUT_LINE('Mileage:         ' ||v_Mileage||'km');
    DBMS_OUTPUT.PUT_LINE('Manufacturer:    ' ||v_Manufacturer);
    
END;
/

--Question 5 with Cursor Declaration
 ...
 DECLARE
    ...
    CURSOR info 
    IS
    SELECT Vehicle_Type, Mileage, Manufacturer 
    FROM vehicle
    WHERE mileage > 700000;
...
OPEN info;
    LOOP
    FETCH info INTO  v_Vehicle, v_Mileage, v_Manufacturer;
...
--Question 6 with no Cursor Declaration

 DECLARE
    v_Vehicle Vehicle.vehicle_type%TYPE;
    v_OldMileage Vehicle.mileage%TYPE;
    v_NewMileage Vehicle.mileage%TYPE;
    
BEGIN
    SELECT vehicle_type, mileage , mileage + 983 INTO v_Vehicle, v_OldMileage, v_NewMileage
    from Vehicle
    where vin_number = '1ZA35868540';
        
    dbms_output.put_line('VEHICLE:                  ' ||v_vehicle);
    dbms_output.put_line('ORIGINAL MILEAGE:         ' ||v_oldMileage||'km');
    dbms_output.put_line('NEW MILEAGE:              ' ||v_newMileage ||'km');
END;
/

--Question 6 with Cursor Declaration
 ...
 DECLARE
    ...
    CURSOR info 
    IS
        SELECT vehicle_type, mileage , mileage + 983 
        from Vehicle
        where vin_number = '1ZA35868540';
...
OPEN info;
    LOOP
    FETCH info INTO v_Vehicle, v_OldMileage, v_NewMileage;
    EXIT WHEN info%NOTFOUND;
...
--Question 7 using Cursor FOR LOOP

DECLARE

v_Customer Customer.First_Name%TYPE;
v_Date DATE;
v_Desciption Delivery_Items.Description%TYPE;
...
IS
    ...
    INNER JOIN Driver_Deliveries dd ON dd.Delivery_Item_ID = di.Delivery_Item_ID
    INNER JOIN Vehicle v ON v.Vin_Number = dd.Vin_Number
    WHERE b.Bill_Date = '10 November 2016';
    
BEGIN
FOR rec IN info
    LOOP
    v_Customer := rec.Customer;
    v_Date := rec.Bill_Date;
    v_Desciption := rec.Description;
    v_Type := rec.Vehicle_Type;
    
        DBMS_OUTPUT.PUT_LINE('CUSTOMER     :'||v_Customer);
        DBMS_OUTPUT.PUT_LINE('BILL DATE    :'||v_Date);
        DBMS_OUTPUT.PUT_LINE('DESCRIPTION  :'||v_Desciption);
